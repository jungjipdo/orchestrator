...
export function ReleasePlanView() {
    ...
    const handleDrop = useCallback((e: React.DragEvent) => {
        ...
        try {
            ...
            // Optimistic: 로컬 상태 먼저 변경 후 API 호출
            if (type === 'plan') {
    ...
    const handleBacklogDragLeave = useCallback(() => {
    ...
    const handleBulkDelete = useCallback(async () => {
        ...
        for (const id of targets) {
            ...
            if (plans.find(p => p.id === id)) {
                await deletePlan(id)
            } else if (projects.find(p => p.id === id)) {
                    ...
                    backlogProjects.map(project => {
                        ...
                        return (
                            <Card
                                ...
                                onDragEnd={handleDragEnd}
                                onMouseDown={handleCardMouseDown}
                                onMouseUp={(e) => deleteMode ? undefined : (() => { if ((e.target as HTMLElement).closest('button, a, input, select')) return; if (mouseDownPos.current) { const dx = Math.abs(e.clientX - mouseDownPos.current.x); const dy = Math.abs(e.clientY - mouseDownPos.current.y); if (dx > 5 || dy > 5) return; } setOverlayProjectId(project.id) })()}
                                onClick={() => deleteMode && setDeleteTargets(prev => {
                                    const next = new Set(prev)
    ...
    return (
        <div className="flex gap-6 h-full">
            {/* ═══ 왼쪽: Plans & Projects ═══ */}
            <div
                className={`w-1/2 shrink-0 overflow-y-auto space-y-6 pr-4 border-r scrollbar-hide rounded-lg transition-all ${dragOverBacklog ? 'ring-2 ring-primary bg-primary/5' : ''}`}
            ...
            <div className="w-1/2 overflow-y-auto space-y-6 scrollbar-hide">
                ...
                >
                    ...
                    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 p-2 rounded-lg border-2 border-dashed transition-all ${dragOverActive
